#!/usr/bin/env python3
"""Cross-platform model downloader for openclaw-voice-io.

Downloads:
- Sherpa-ONNX offline ASR (paraformer zh-small)
- (Optional) Piper voice model (zh_CN huayan x_low) for Feishu sender

Usage:
  python scripts/download_models.py
  python scripts/download_models.py --models-dir ./models
  python scripts/download_models.py --stt-only
...
def main() -> int:
    ...
    ap.add_argument("--stt-only", action="store_true", help="Only download STT models (skip Piper)")
    args = ap.parse_args()
